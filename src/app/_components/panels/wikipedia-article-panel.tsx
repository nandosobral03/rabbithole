...
import { ArrowLeft, ChevronLeft, ChevronRight, ExternalLink, Trash2, X } from "lucide-react";
...
export function WikipediaArticlePanel({
  ...
  useEffect(() => {
    if (selectedNode && isDetailPanelOpen && !isCollapsed) {
      ...
      setTimeout(() => {
        const contentContainer = document.querySelector(".wikipedia-article-panel-content");
        if (contentContainer) {
    ...
    >
      ...
      {loadingLinks.size > 0 && !isCollapsed && (
        <div className="absolute inset-0 z-30 flex items-center justify-center bg-background/90 backdrop-blur-sm">
          <div className="flex flex-col items-center gap-3 p-4 text-center sm:p-8">
            ...
            <div className="font-medium text-muted-foreground text-sm">Loading article...</div>
          </div>
        </div>
      )}

      {/* Resize Handle - Hidden on mobile */}
      {!isCollapsed && panelWidth > 400 && <div className="absolute top-0 left-0 h-full w-1 cursor-col-resize bg-border transition-colors duration-200 hover:bg-primary" onMouseDown={onPanelResize} />}
        ...
        <>
          ...
          <div className="flex items-center justify-between border-border border-b p-3 sm:p-6">
            <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-3">
              <h3 className="min-w-0 flex-1 truncate font-semibold text-card-foreground text-lg sm:text-xl">{selectedNode.title}</h3>
              {/* Root Node Badge */}
              {isRootNode && (
                <span className="rounded-full bg-green-100 px-2 py-1 font-medium text-green-800 text-xs" title="This is a root node (starting point of exploration)">
                  Root
                </span>
              )}
            </div>
            ...
            <div className="flex flex-shrink-0 items-center gap-1 sm:gap-2">
              ...
              <Button variant="ghost" size="sm" onClick={() => window.open(selectedNode.url, "_blank")} className="flex items-center gap-1 text-muted-foreground hover:text-card-foreground sm:gap-2" title="View on Wikipedia">
              ...
              {/* Close Button */}
              <Button variant="ghost" size="sm" onClick={() => setIsDetailPanelOpen(false)} className="flex items-center gap-1 sm:gap-2" title="Close panel">
          ...
          <div className="wikipedia-article-panel-content flex-1 overflow-y-auto p-3 sm:p-6">
            <div className="space-y-4">
              {/* Article Content */}
              <div>
                <WikipediaArticleViewer htmlContent={selectedNode.fullHtml} title={selectedNode.title} loadingLinks={loadingLinks} onLinkClick={onLinkClick} onMiddleClick={onMiddleClick} />
              </div>
            </div>
          </div>
        </>
      ...
      {isCollapsed && (
        <div className="flex h-full items-center justify-center p-2">
          <div className="-rotate-90 mb-4 transform whitespace-nowrap font-medium text-muted-foreground text-sm">{selectedNode.title}</div>
        </div>
      )}
      ...
      {!selectedNode.fullHtml && !isCollapsed && (
        <div className="flex h-full items-center justify-center">
          <div className="text-muted-foreground italic">Loading article content...</div>
        </div>
      )}
    </div>
  );
