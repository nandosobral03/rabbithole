import { BarChart3, ExternalLink, Mouse, MousePointer, MousePointer2, Rocket, Star, Users } from "lucide-react";
...
export default async function HomePage() {
  ...
  const [stats, recentRabbitholes, articleOfTheDay] = await Promise.all([api.rabbithole.getRabbitholeStats(), api.rabbithole.getRecentRabbitholes({ limit: 3 }), fetchArticleOfTheDay()]);

  const shuffledTopics = shuffleWithDateSeed(POPULAR_STARTING_POINTS).slice(0, 6);
            ...
            <div className="order-2 space-y-4 lg:order-1 lg:col-span-3 lg:space-y-6">
              ...
              {articleOfTheDay ? (
                ...
                <div className="rounded-lg border border-border bg-card p-3 shadow-sm sm:p-4">
                  ...
                  <p className="text-muted-foreground text-xs">Could not load today's featured article</p>
                </div>
              )}
                    ...
                    {recentRabbitholes.slice(0, 3).map((rabbithole) => (
                      <Link key={rabbithole.id} href={`/${rabbithole.id}`} className="block w-full rounded border border-chart-3/20 bg-chart-3/10 p-2 text-left text-chart-3 transition-colors hover:bg-chart-3/20 sm:p-3">
                        ...
                        <div className="mt-1 text-chart-3/80 text-xs">
                          ...
                          {rabbithole.creatorName && <span> • by {rabbithole.creatorName}</span>}
                        </div>
                      </Link>
                    ))}
                  </div>
                ...
                <div className="rounded-lg border border-border bg-card p-3 shadow-sm sm:p-4">
                  ...
                  <div className="space-y-1.5 text-sm sm:space-y-2">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{stats.totalArticles.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Connections</span>
                      <span className="font-medium">{stats.totalConnections.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
              )}
