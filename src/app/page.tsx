						...
						<div className="order-3 space-y-4 lg:col-span-3 lg:space-y-6">
									...
									<div className="space-y-1.5 sm:space-y-2">
										{recentRabbitholes.slice(0, 3).map((rabbithole) => (
											<Link
												key={rabbithole.id}
												href={`/${rabbithole.id}`}
												className="block w-full rounded border border-chart-3/20 bg-chart-3/10 p-2 text-left text-chart-3 transition-colors hover:bg-chart-3/20 sm:p-3"
											>
							...
							{stats && (
								<div className="rounded-lg border border-border bg-card p-3 shadow-sm sm:p-4">
									<Link
										href="/analytics"
										className="mb-2 flex items-center justify-between transition-colors hover:text-foreground sm:mb-3"
									>
