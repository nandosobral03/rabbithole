	...
	fetchPageWithLinks: publicProcedure
		...
		.mutation(async ({ input }): Promise<WikipediaPageData> => {
			try {
				...
				const wikipediaUrlPattern =
					/(?:https?:\/\/)?(?:www\.)?(?:[a-z]+\.)?wikipedia\.org\/wiki\/(.+)/i;
				...
				if (!pageResponse.ok) {
					...
					console.error(
						`Wikipedia API error for "${cleanTitle}":`,
						pageResponse.status,
						errorText,
					);
					throw new Error(
						`Wikipedia page not found: "${cleanTitle}". Status: ${pageResponse.status}`,
					);
				}
				...
				if (!linksResponse.ok) {
					console.error(
						`Failed to fetch links for "${cleanTitle}":`,
				...
				return {
					...
					links,
					url:
						pageData.content_urls?.desktop?.page ||
						`https://en.wikipedia.org/wiki/${encodedTitle}`,
				};
			} catch (error) {
				throw new Error(
					error instanceof Error
						? `Failed to fetch Wikipedia data: ${error.message}`
		...
		.mutation(async ({ input }): Promise<WikipediaFullPageData> => {
			try {
				...
				if (!htmlResponse.ok) {
					...
					console.error(
						`Wikipedia HTML API error for "${cleanTitle}":`,
						htmlResponse.status,
						errorText,
					);
					throw new Error(
						`Wikipedia page not found: "${cleanTitle}". Status: ${htmlResponse.status}`,
					);
				}
