...
export const rabbitholeRouter = createTRPCRouter({
	...
	// Create a new rabbit hole based on an existing one (fork)
	fork: publicProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const original = await ctx.db.query.sharedRabbitholes.findFirst({
				where: eq(sharedRabbitholes.id, input.originalId),
			});

			if (!original) {
				throw new Error("Original rabbit hole not found");
			}
			...
			// Collect analytics data for the fork
			await collectAnalytics(ctx.db, id, graphData);

			return { id, expiresAt };
		}),

	// Get metadata about a rabbit hole without extending expiration
	metadata: publicProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			// Check if expired
			if (rabbithole.expiresAt < new Date()) {
				throw new Error("Rabbit hole has expired");
			}
