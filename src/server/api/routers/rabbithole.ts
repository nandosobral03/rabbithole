...
import { articleAnalytics, connectionAnalytics, nodeAnalytics, sharedRabbitholes } from "~/server/db/schema";
...
export const rabbitholeRouter = createTRPCRouter({
  ...
  load: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      await ctx.db.delete(sharedRabbitholes).where(lt(sharedRabbitholes.expiresAt, new Date()));
      ...
      const graphData = JSON.parse(rabbithole.graphData) as z.infer<typeof GraphDataSchema>;
  ...
  getPopularArticles: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      return await ctx.db.query.articleAnalytics.findMany({
        orderBy: [desc(articleAnalytics.totalAppearances), desc(articleAnalytics.totalConnections)],
        limit: input.limit,
      });
    }),
  ...
  getRabbitholeStats: publicProcedure.query(async ({ ctx }) => {
    ...
    const totalArticles = await ctx.db.select({ count: sql<number>`count(*)` }).from(articleAnalytics);

    const totalConnections = await ctx.db.select({ count: sql<number>`count(*)` }).from(connectionAnalytics);
  ...
  getMostConnectedArticles: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      return await ctx.db.query.articleAnalytics.findMany({
        orderBy: [desc(articleAnalytics.averageConnections), desc(articleAnalytics.totalConnections)],
        limit: input.limit,
      });
    }),
...
async function collectAnalytics(db: typeof import("~/server/db").db, rabbitholeId: string, graphData: z.infer<typeof GraphDataSchema>) {
  const now = new Date();
  ...
  for (const link of graphData.links) {
    const existing = await db.query.connectionAnalytics.findFirst({
      where: and(eq(connectionAnalytics.sourceArticle, link.source), eq(connectionAnalytics.targetArticle, link.target)),
    });
